...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # When one is a container and the container does not already have an ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            while self.lower_bound <= ind <= self.upper_bound and not current_place.is_hive:
...
class FireAnt(Ant):
    ...
    # BEGIN Problem 5
    implemented = True   # Change to True to view in the GUI
    # How to obtain current place of fire = By accessing place instance attribute, which is a Place object
    # How to obtain all bees in a given place = By accessing the .bees instance attribute, which is a list of Bee objects
    # I should iterate over a copy of the list to avoid skipping elements
    # END Problem 5
...
class HungryAnt(Ant):
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3 #Class
    implemented = True

    def __init__(self, health=1):
        super().__init__(health)
        self.turns_to_chew = 0    #Instance attribute, bcz they chew independently of each other.

    # A random bee from its place (bcz multiple can be there)
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            self.turns_to_chew -= 1
        else:
            if self.place.bees:
                random_beee = random_bee(self.place.bees)
                Insect.reduce_health(random_beee, random_beee.health)
    ...
    def action(self, gamestate):
        ...
        while current_place:
            if current_place.ant and current_place.ant.is_container:
                Ant.double(current_place.ant)
                if current_place.ant.ant_contained:
                    Ant.double(current_place.ant.ant_contained)
            elif current_place.ant:
                Ant.double(current_place.ant)
